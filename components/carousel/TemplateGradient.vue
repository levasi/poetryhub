...
<template>
  <div class="carousel-canvas template-gradient relative flex flex-col overflow-hidden text-white antialiased" :style="{
    ...
    <template v-if="variant === 'cover'">
      <div class="relative flex h-full flex-col items-center justify-center text-center">
        <img v-if="avatarUrl" :src="avatarUrl" alt="" class="rounded-full object-cover shadow-xl ring-4 ring-white/40"
          width="236" height="236" :style="{
            width: `${coverAvatarPx}px`,
            height: `${coverAvatarPx}px`,
            marginBottom: `${coverGap1}px`,
          }" />
        <p class="font-bold leading-[1.1] drop-shadow-md" :style="{ fontSize: `${coverTitlePx}px`, maxWidth: '920px' }">
          {{ title }}
        </p>
        <p v-if="writtenYearLine?.trim()" class="tabular-nums text-white/70" :style="{
        ...
        <div class="h-1 rounded-full bg-white/35"
          :style="{ marginTop: `${coverGap1}px`, width: `${coverDividerW}px` }" />
        <p class="font-medium text-white/85" :style="{ fontSize: `${coverAuthorPx}px`, marginTop: `${coverGap1}px` }">
          {{ author }}
        </p>
        <template v-if="hasAuthorMeta">
          <p v-if="authorNationality?.trim()" class="text-white/70" :style="{
            fontSize: `${coverMetaPx}px`,
            marginTop: `${coverMetaGap}px`,
            letterSpacing: '0.02em',
          }">
          ...
          <p v-if="authorLifespan?.trim()" class="tabular-nums text-white/70" :style="{
            fontSize: `${coverMetaPx}px`,
    ...
    <template v-else-if="variant === 'body'">
      <div class="relative flex h-full flex-col justify-center px-6 text-center text-white/95" :style="{
