...
const coverTitlePx = computed(() => Math.round(60 * props.titleScale))
...
const coverTagPx = computed(() => Math.round(12 * props.titleScale))
...
<template>
  <div
    class="carousel-canvas template-dark relative flex flex-col overflow-hidden antialiased"
    :style="{
  ...
  >
    ...
    <template v-if="variant === 'cover'">
      <div class="relative flex h-full flex-col items-center justify-center text-center">
        <img
          ...
          :src="avatarUrl"
          alt=""
          class="rounded-full object-cover shadow-xl ring-4 ring-gold-500/35"
          width="240"
          height="240"
          :style="{
        ...
        />
        <p
          class="font-semibold leading-[1.12] text-ink-50"
          :style="{ fontSize: `${coverTitlePx}px`, maxWidth: '920px' }"
        >
          {{ title }}
        </p>
        <div
          class="h-px bg-gradient-to-r from-transparent via-gold-500/80 to-transparent"
          :style="{ marginTop: `${coverGap1}px`, width: `${coverDividerW}px` }"
        />
        <p
          class="font-medium tracking-wide text-ink-300"
        ...
        <template v-if="hasAuthorMeta">
          ...
          <p
            ...
            class="tabular-nums text-ink-400"
            :style="{
              fontSize: `${coverMetaPx}px`,
        ...
        <p
          class="uppercase tracking-[0.4em] text-ink-500"
    ...
    <template v-else-if="variant === 'body'">
      <div
        class="relative flex h-full flex-col justify-center px-4 text-center text-ink-100"
        :style="{
    ...
    <template v-else>
      <div class="relative flex h-full flex-col items-center justify-center px-10 text-center">
        <p
          class="text-[2.65rem] font-semibold leading-snug text-ink-50"
