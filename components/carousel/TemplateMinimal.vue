...
const coverTagPx = computed(() => Math.round(15 * props.titleScale))
...
<template>
  <div
    class="carousel-canvas template-minimal relative flex flex-col overflow-hidden bg-white text-ink-900 antialiased"
    :style="{
  ...
  >
    <template v-if="variant === 'cover'">
      <div class="flex h-full flex-col items-center justify-center text-center">
        <img
          ...
          :src="avatarUrl"
          alt=""
          class="rounded-full object-cover shadow-lg ring-4 ring-ink-200/90"
          width="236"
          height="236"
          :style="{
        ...
        />
        <p
          class="font-semibold leading-[1.12] text-ink-900"
          :style="{ fontSize: `${coverTitlePx}px`, maxWidth: '920px' }"
        >
          {{ title }}
        </p>
        <div
          class="h-px bg-ink-300"
          :style="{ marginTop: `${coverGap1}px`, width: `${coverDividerW}px` }"
        />
        <p
          class="font-medium tracking-wide text-ink-600"
        ...
        <template v-if="hasAuthorMeta">
          ...
          <p
            ...
            class="tabular-nums text-ink-500"
            :style="{
              fontSize: `${coverMetaPx}px`,
        ...
        <p
          class="uppercase tracking-[0.35em] text-ink-400"
    ...
    <template v-else-if="variant === 'body'">
      <div
        class="flex h-full flex-col justify-center px-2 text-center text-ink-800"
        :style="{
    ...
    <template v-else>
      <div class="flex h-full flex-col items-center justify-center px-8 text-center">
        <p
          class="text-4xl font-semibold leading-snug text-ink-900"
