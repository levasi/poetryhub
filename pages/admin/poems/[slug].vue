  ...
  <div class="w-full">
    <div class="mb-6 flex items-center justify-between gap-4">
      <div class="flex min-w-0 flex-1 items-center gap-4">
        <NuxtLink to="/admin/poems" class="shrink-0 text-sm text-content-muted hover:text-content">{{ t('admin.poemForm.backPoems') }}</NuxtLink>
        <h1 class="min-w-0 truncate font-serif text-xl font-bold text-content">{{ t('admin.poemForm.editTitle', { title: form.title }) }}</h1>
      </div>
    ...
    <form class="space-y-5" @submit.prevent="submit">
      ...
      <div>
        <label class="mb-1.5 block text-xs font-medium text-content-muted">{{ t('admin.poemForm.title') }}</label>
        <input v-model="form.title" type="text" required class="admin-input" />
      </div>

      <div>
        <label class="mb-1.5 block text-xs font-medium text-content-muted">{{ t('admin.poemForm.author') }}</label>
      ...
      <div>
        <label class="mb-1.5 block text-xs font-medium text-content-muted">{{ t('admin.poemForm.content') }}</label>
      ...
      <div>
        <label class="mb-1.5 block text-xs font-medium text-content-muted">{{ t('admin.poemForm.sourceUrl') }}</label>
      ...
      <label class="flex cursor-pointer items-center gap-3">
        ...
        <span class="text-sm text-content-secondary">{{ t('admin.poemForm.featuredEdit') }}</span>
      </label>
      ...
      <div class="flex gap-3 pt-2">
        ...
        <NuxtLink to="/admin/poems" class="rounded-lg border border-edge-subtle bg-surface-raised px-5 py-2.5 text-sm text-content-muted hover:border-edge hover:text-content">{{ t('admin.poemForm.cancel') }}</NuxtLink>
      </div>
    </form>
  </div>
</template>
