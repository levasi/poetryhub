...
/* TEMP: email/password login — restore by merging into one `useAuth()` destructure:
 * const { login, loading, isLoggedIn, loginWithGoogle } = useAuth()
 * const form = reactive({ email: '', password: '' })
 * const error = ref('')
 * const showPassword = ref(false)
 * async function submit() { ... see git history / commented template block in this file }
 */
</script>
  ...
  <div class="relative flex min-h-screen items-center justify-center bg-surface-base px-4">
    ...
    <div class="w-full max-w-md">
      ...
      <div class="rounded-xl border border-edge-subtle bg-surface-raised p-8 shadow-ds-card">
        ...
        <!-- TEMP: email/password sign-in (Google only for now). Restore with script block above.
        ...
        <form class="space-y-5" @submit.prevent="submit">
          <div>
            <label class="mb-1.5 block text-xs font-medium uppercase tracking-widest text-content-secondary">{{ t('auth.email') }}</label>
            <input v-model="form.email" type="email" placeholder="you@example.com" required autocomplete="email" class="w-full rounded-lg border border-edge-subtle bg-surface-subtle px-4 py-3 text-sm text-content placeholder-content-soft outline-none transition focus:border-brand focus:ring-2 focus:ring-brand/20" />
          </div>
          <div>
            <label class="mb-1.5 block text-xs font-medium uppercase tracking-widest text-content-secondary">{{ t('auth.password') }}</label>
            <div class="relative">
              <input v-model="form.password" :type="showPassword ? 'text' : 'password'" :placeholder="t('auth.passwordPlaceholder')" required autocomplete="current-password" class="w-full rounded-lg border border-edge-subtle bg-surface-subtle px-4 py-3 pr-11 text-sm text-content placeholder-content-soft outline-none transition focus:border-brand focus:ring-2 focus:ring-brand/20" />
              <button type="button" tabindex="-1" class="absolute right-3 top-1/2 -translate-y-1/2 text-content-soft transition hover:text-content" :aria-label="showPassword ? t('auth.hidePassword') : t('auth.showPassword')" @click="showPassword = !showPassword">
          ...
          <button type="submit" :disabled="loading" class="ds-btn-primary w-full justify-center py-3 text-sm disabled:opacity-50">{{ loading ? t('auth.signingIn') : t('auth.signIn') }}</button>
        </form>
        -->
