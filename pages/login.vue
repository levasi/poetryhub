  ...
  <div class="relative flex min-h-screen items-center justify-center bg-ink-50 px-4">
    ...
    <div class="w-full max-w-md">
      ...
      <div class="mb-8 text-center">
        ...
        <p class="mt-2 text-sm text-ink-600">{{ t('auth.signInSubtitle') }}</p>
      </div>
      ...
      >
        ...
        <p class="mt-6 text-center text-sm text-ink-600">
          {{ t('auth.noAccount') }}
          <NuxtLink to="/signup" class="text-gold-800 underline hover:text-gold-900">{{ t('auth.signUpLink') }}</NuxtLink>
        </p>
      </form>
    </div>
  </div>
