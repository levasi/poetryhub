...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<p class="mt-2 text-sm text-ink-600">${ssrInterpolate(unref(t)("auth.signUpSubtitle"))}</p></div><form class="rounded-xl border border-ink-200 bg-white p-8 shadow-lg"><div class="space-y-5"><div><label class="mb-1.5 block text-xs font-medium uppercase tracking-widest text-ink-600">${ssrInterpolate(unref(t)("auth.nameOptional"))}</label><input${ssrRenderAttr("value", unref(form).name)} type="text"${ssrRenderAttr("placeholder", unref(t)("auth.yourName"))} autocomplete="name" class="w-full rounded-lg border border-ink-200 bg-ink-50/50 px-4 py-3 text-sm text-ink-900 placeholder-ink-400 outline-none transition focus:border-gold-400 focus:ring-2 focus:ring-gold-300/40"></div><div><label class="mb-1.5 block text-xs font-medium uppercase tracking-widest text-ink-600">${ssrInterpolate(unref(t)("auth.email"))}</label><input${ssrRenderAttr("value", unref(form).email)} type="email" placeholder="you@example.com" required autocomplete="email" class="w-full rounded-lg border border-ink-200 bg-ink-50/50 px-4 py-3 text-sm text-ink-900 placeholder-ink-400 outline-none transition focus:border-gold-400 focus:ring-2 focus:ring-gold-300/40"></div><div><label class="mb-1.5 block text-xs font-medium uppercase tracking-widest text-ink-600">${ssrInterpolate(unref(t)("auth.password"))}</label><input${ssrRenderAttr("value", unref(form).password)} type="password"${ssrRenderAttr("placeholder", unref(t)("auth.passwordMin"))} required autocomplete="new-password" class="w-full rounded-lg border border-ink-200 bg-ink-50/50 px-4 py-3 text-sm text-ink-900 placeholder-ink-400 outline-none transition focus:border-gold-400 focus:ring-2 focus:ring-gold-300/40"></div><div><label class="mb-1.5 block text-xs font-medium uppercase tracking-widest text-ink-600">${ssrInterpolate(unref(t)("auth.confirmPassword"))}</label><input${ssrRenderAttr("value", unref(form).confirm)} type="password"${ssrRenderAttr("placeholder", unref(t)("auth.repeatPassword"))} required autocomplete="new-password" class="w-full rounded-lg border border-ink-200 bg-ink-50/50 px-4 py-3 text-sm text-ink-900 placeholder-ink-400 outline-none transition focus:border-gold-400 focus:ring-2 focus:ring-gold-300/40"></div>`);
      if (unref(error)) {
        _push(`<p class="text-sm text-red-600">${ssrInterpolate(unref(error))}</p>`);
      } else {
        _push(`<!---->`);
      }
      _push(`<button type="submit"${ssrIncludeBooleanAttr(unref(loading)) ? " disabled" : ""} class="w-full rounded-lg bg-gold-600 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-gold-700 disabled:opacity-50">${ssrInterpolate(unref(loading) ? unref(t)("auth.creating") : unref(t)("auth.createAccount"))}</button></div><p class="mt-6 text-center text-sm text-ink-600">${ssrInterpolate(unref(t)("auth.haveAccount"))} `);
      _push(ssrRenderComponent(_component_NuxtLink, {
        to: "/login",
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`${ssrInterpolate(unref(t)("auth.signInLink"))}`);
          } else {
            return [
              createTextVNode(toDisplayString(unref(t)("auth.signInLink")), 1)
            ];
          }
        }),
        _: 1
...
//# sourceMappingURL=signup-XIYaC749.mjs.map
